...
{    
    ...
    {
        if (!$this->product) {
            $this->product = $this->api->getProduct('travel');
            if ($this->product) {
    ...
    {
        $this->calculated_data = $this->api->calculateProduct('travel', $params);
    ...
    public function getMinCalculatedPrice($companies)
    {
        ...
        foreach ($this->calculated_data as $element) {
                if (min((array)$element->price_special) != 0 &&
                    isset($companies[$element->companyInfo->alias]) &&
                    $element->companyInfo->active == 1
                ) {
                    $cur_price = ceil(min((array)$element->price_special)+$element->ns->price_special);
